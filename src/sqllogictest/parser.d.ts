...
/**
 * Anything that can successfully be converted to a string with `String()` so
...
export interface GrammarSourceObject {
  ...
  /**
   * If specified, allows the grammar source to be embedded in a larger file at
   * some offset.
   */
...
/**
 * Expected some other input. These are specified with a rule's "human-readable
 * name", or with the `expected(message, location)` function.
 */
...
export interface ParseOptions<T extends StartRuleNames = "start"> {
   ...
   * created by the parser. For example, this can be path to the parsed file or
   * even the File object.
   */
